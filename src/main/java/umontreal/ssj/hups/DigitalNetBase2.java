...
public class DigitalNetBase2 extends DigitalNet {
	 ...
	 * digits, in dimension `dim`. We will give the generator
	 * matrices @f$\mathbf{C}@f$ are @f$k\times dim@f$, as a vector ordered by
	 * blocks of k components for each dimension. Restrictions: @f$0\le
	 * k\le30@f$, @f$k\le w@f$, and `dim` @f$ \le360@f$.can be changed (i took
	 * standard values from Sobol)
	 *
	 * 
	...
	/**
	 * This method is essential for recreating a Digital net from the optimized
	 * generator matrices.
	 */
	...
	private void init1(int k, int r, int w, int dim, int[] C) {
		if (filename == null)
			if ((dim < 1) || (dim > MAXDIM))
				throw new IllegalArgumentException("Dimension for DigitalNetBase2 must be > 0 and <= " + MAXDIM);
			else if (dim < 1)
				throw new IllegalArgumentException("Dimension for DigitalNetBase2 must be > 0");

		if (r < k || w < r || w > MAXBITS || k >= MAXBITS)
			throw new IllegalArgumentException("One must have k < 31 and k <= r <= w <= 31 for DigitalNetBase2");
		numCols = k;
	...
	/**
	 * Same as printGeneratorMatricesTrans reutns a string Prints the generator
	 * matrices transposed in the form of integers for dimensions 1 to @f$s@f$. Each
	 * integer corresponds to one column of bits.
	 */
	public String getGeneratorMatricesTransAsString(int s) {
		StringBuilder sb = new StringBuilder();
		for (int j = 0; j < s; j++) {
			for (int c = 0; c < numCols; c++) {
				sb.append(genMat[j * numCols + c]).append(" ");
				if (c < numCols - 1) { // Check to avoid appending a space at the end of the line
					sb.append(" ");
				}
			}
	 ...
	 * scrambling matrix is represented as an integer less than `w`, just like for
	 * Cj. The nonzero part of L_j will be r x numRows, i.e., the scrambled
	 * generating matrices will have `r` nonzero rows only. They are put in
	 * `genMat`. The algo is described in the document "latnetbuider notes".
	 */

	public void leftMatrixScrambleBis(int r, RandomStream stream) {
	...
	// same as leftMatrixScrambleBis (int r, RandomStream stream) but the lms is w
	// \times w
	public int[][] leftMatrixScrambleBis(RandomStream stream) {
		int j, d; // dimension j, subdiagonal d.
		final int twoRm1 = 1 << (outDigits - 1);
		...
		// Constructs the lower-triangular scrambling matrices L_j, w by w.
		// scrambleMat[j][l] contains column l of L_j as a single integer.
		int[][] scrambleMat = new int[dim][numCols];
		for (j = 0; j < dim; j++) {
			for (int c = 0; c < numCols; c++) {
				scrambleMat[j][c] = (twoRm1 + stream.nextInt(0, twoRm1 - 1)) >> c;
				for (int l = 0; l < numCols; l++) {
					genMat[j * numCols + l] ^= ((originalMat[j * numCols + l] >> (outDigits - c - 1)) & 1)
							* scrambleMat[j][c];
				}
...
//	public void leftMultiplyMatBis3(int j, int[] Mj) {
//		if (originalMat == null) {
//			originalMat = genMat;
//			genMat = new int[dim * numCols];
//		}
//		int c, d;
//		for (c = 0; c < outDigits; c++) {
//			int L = Mj[c];
//			for (int l = 0; l < numCols; l++) {
//				genMat[j * numCols + l] ^= ((originalMat[j * numCols + l] >> (outDigits - c - 1)) & 1) * L;
//			}
//		}
//	}
//
//	// similar to leftMultiplyMat
//	public void leftMultiplyMatBis2(int j, int[] Mj) {
//		if (originalMat == null) {
//			originalMat = genMat;
//			genMat = new int[dim * numCols];
//		}
...
//	}
	...
	// similar to leftMultiplyMat
	public void leftMultiplyMatBisV2(int j, int[] Mj, int d) {
		if (originalMat == null) {
			originalMat = genMat;
			genMat = new int[dim * numCols];
		}
...
//	public int[] GenerateleftMatrixScrambleBis(RandomStream stream) {
//		int c;
...
//	}
	...
	protected class DigitalNetBase2Iterator extends DigitalNetIterator {
		...
		/**
		 * return the coordinate in int format
		 */
		public int nextInt() {
			if (curPointIndex >= numPoints || curCoordIndex >= dimS)
				outOfBounds();
			if (digitalShift == null)
				return cachedCurPoint[curCoordIndex++];
			else
				return cachedCurPoint[curCoordIndex++];
		}
